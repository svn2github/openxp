...
public:
	tagData(){
		memset(chSpell,0,sizeof(chSpell));
		memset(chWordS,0,sizeof(chWordS));
	}
public:
	char chSpell[MAX_SPELL];
	char chWordS[MAX_WORDS];
}WORD_DATA,*pWORD_DATA;

static WORD_DATA  g_Word[480];

static void InitWordData()
{
	CStdioFile file;
	file.Open("words.lj",CFile::modeRead);

	int nSize = 0;
	CString strText;
	while(file.ReadString(strText))
	{
		int nDot = strText.Find(',');
		CString strLeft = strText.Left(nDot);
		CString strRight = strText.Right(strText.GetLength()-nDot-1);
		strcpy(g_Word[nSize].chSpell,strLeft);
		strcpy(g_Word[nSize].chWordS,strRight);
 		nSize++;
	}
	file.Close();
};
...
HWordToSpell::HWordToSpell(void)
{
	m_strPart = " ";
	InitWordData();
}	
...
CString HWordToSpell::Word2Spell( CString strIn )
{
	for (int i = 479; i >= 0; i--)
	{
		CString strSpell,strWord;
		strSpell.Format("%s",g_Word[i].chSpell);
		strWord.Format("%s",g_Word[i].chWordS);
		for (int i = 0; i < strWord.GetLength(); i++)
		{
			if (i%2 != 0) continue;
			CString strIndex;
			strIndex.Format("%s",strWord.Mid(i,2));
			if (strIndex == strIn) return strSpell;
		}
	}
	return "";
}

CString HWordToSpell::ParseWords( CString strWords )
{
	CString strResult = " ";
	int nLenght = strWords.GetLength();
	for (int i = 0; i < nLenght; i++)
	{
		if (i%2 != 0) continue;
		CString strStep = strWords.Mid(i,2);
		strStep = Word2Spell(strStep);
		strResult += strStep;
		if (i == (nLenght-2)) break;
		strResult += m_strPart;
	}
	return strResult;
}