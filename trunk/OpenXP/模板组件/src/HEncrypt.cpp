...
//生成密文
void HMD5Encrypt::EncryptData(LPCTSTR pszSrcData, TCHAR szMD5Result[16])
{
	//加密密文
	CMD5 MD5Encrypt;
	UCHAR szResult[16];
...
//MD5文件摘要
MD5VAL HMD5Encrypt::md5File(FILE * fpin)
{
	if(!Buffer)
...
CString HMD5Encrypt::ValueFileMD5(CString file)
{
	MD5VAL val;
	CString result;
	char chtmp[1024];
	...
	{
		val = md5File(fp);	
		sprintf(chtmp, "%08x%08x%08x%08x\n", conv(val.a), conv(val.b), conv(val.c), conv(val.d));
		fclose(fp);
	}
	result.Format("%s", chtmp);
	return result;
...
//生成密文
WORD HXOREncrypt::EncryptData(LPCTSTR pszSrcData, LPTSTR pszEncrypData, WORD wSize)
{
	//效验参数
	ASSERT(wSize>0);
	...
	for (WORD i=1;i<SafeCountArray(szRandKey);i++) szRandKey[i]=rand()%255;
...
//解开密文
WORD HXOREncrypt::CrevasseData(LPCTSTR pszEncrypData, LPTSTR pszSrcData, WORD wSize)
{
	//效验参数
	ASSERT(wSize>0);
	ASSERT(pszSrcData!=NULL);
