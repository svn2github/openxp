...
CShockwaveFlash::CShockwaveFlash():m_bEnableMenu(0)   
{   
} 
...
void CShockwaveFlash::OnRButtonDown(UINT nFlags, CPoint point)
{
	if (!m_bEnableMenu) return;
	CWnd::OnRButtonDown(nFlags, point);
}

 

HFlashPlay::HFlashPlay()
{
}
...
BEGIN_MESSAGE_MAP(HFlashPlay, CWnd)   
	//{{AFX_MSG_MAP(HFlashPlay)    
	ON_WM_CREATE()   
	//}}AFX_MSG_MAP    
END_MESSAGE_MAP()   

BOOL HFlashPlay::CreateFlashWnd(CWnd* pParentWnd, CRect rect)   
{   
	...
	LPCTSTR lpszBounceClass =   
		...
		NULL);   
	return CWnd::CreateEx(0,lpszBounceClass,NULL,WS_CHILD|WS_VISIBLE|WS_CLIPSIBLINGS|WS_OVERLAPPED,    
		rect.left, rect.top, rect.right, rect.bottom, pParentWnd->GetSafeHwnd(), NULL);
}   

int HFlashPlay::OnCreate(LPCREATESTRUCT lpCreateStruct)    
{   
...
void HFlashPlay::CreateShockWnd()
{
	CRect rc;   
	GetClientRect(&rc);   
...
void HFlashPlay::StartPlay( BOOL bLoop,BOOL bMenu,long lScaleMode )
{
	m_FlashPlayer.LoadMovie(0, m_lpszPath);//¾ø¶ÔÂ·¾¶   
	m_FlashPlayer.SetBGColor("#C8C8C8");   
	m_FlashPlayer.SetLoop(bLoop);   
	m_FlashPlayer.SetWMode("Transparent");     
	m_FlashPlayer.SetMenu(bMenu);
	m_FlashPlayer.SetEnableMenu(bMenu);
	m_FlashPlayer.SetScaleMode(lScaleMode);   
	m_FlashPlayer.Play(); 
	ShowWindow(SW_SHOW);
}