...
inline int HAnimationGif::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
{
...
inline int HAnimationGif::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
inline int HAnimationGif::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
BEGIN_MESSAGE_MAP(HAnimationGif, CStatic)
	//{{AFX_MSG_MAP(HAnimationGif)
	ON_WM_DESTROY()
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

BOOL HAnimationGif::Load(HGLOBAL hGlobal, DWORD dwSize)
{
...
BOOL HAnimationGif::Draw()
{
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the HAnimationGif::Load() member functions before calling Draw()\n"));
		return FALSE;
	};
...
SIZE HAnimationGif::GetSize() const
{
	return m_PictureSize;
}

BOOL HAnimationGif::Load(LPCTSTR szFileName)
{
	ASSERT(szFileName);
...
BOOL HAnimationGif::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
void HAnimationGif::ResetDataPointer()
{
...
BOOL HAnimationGif::SkipNextGraphicBlock()
{
...
UINT HAnimationGif::GetSubBlocksLen(UINT nStartingOffset) const
{
	UINT nRet = 0;
...
enum HAnimationGif::GIFBlockTypes HAnimationGif::GetNextBlock() const
{
...
BOOL HAnimationGif::SkipNextBlock()
{
	if (!m_pRawData) return FALSE;
...
int HAnimationGif::GetNextBlockLen() const
{
...
UINT WINAPI HAnimationGif::_ThreadAnimation(LPVOID pParam)
{
	ASSERT(pParam);
	HAnimationGif *pPic = reinterpret_cast<HAnimationGif *> (pParam);
...
void HAnimationGif::ThreadAnimation()
{
...
HGLOBAL HAnimationGif::GetNextGraphicBlock(UINT *pBlockLen, 
...
BOOL HAnimationGif::IsGIF() const
{
	return m_bIsGIF;
}

BOOL HAnimationGif::IsAnimatedGIF() const
{
	return (m_bIsGIF && (m_arrFrames.size() > 1));
}

BOOL HAnimationGif::IsPlaying() const
{
	return m_bIsPlaying;
}

int HAnimationGif::GetFrameCount() const
{
	if (!IsAnimatedGIF())
		return 0;
...
COLORREF HAnimationGif::GetBkColor() const
{
	return m_clrBackground;
}

void HAnimationGif::OnPaint() 
{
...
BOOL HAnimationGif::PrepareDC(int nWidth, int nHeight)
{
...
void HAnimationGif::OnDestroy() 
{
	Stop();	
	CStatic::OnDestroy();
}

void HAnimationGif::SetBkColor(COLORREF clr)
{
...
#ifdef GIF_TRACING
void HAnimationGif::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
{
	CFile file;
...
void HAnimationGif::EnumGIFBlocks()
{
	enum GIFBlockTypes nBlock;
...
BOOL HAnimationGif::SetPaintRect(const RECT *lpRect)
{
	return CopyRect(&m_PaintRect, lpRect);
}

BOOL HAnimationGif::GetPaintRect(RECT *lpRect)
{
